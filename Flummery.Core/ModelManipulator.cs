...
namespace Flummery.Core
{
    ...
    public static class ModelManipulator
    {
        ...
        public static void Normalise(ModelMeshPart model)
        {
            ...
            for (int i = 0; i < model.VertexBuffer.Data.Count; i++)
            {
                model.VertexBuffer.ModifyVertexNormal(i, model.VertexBuffer.Data[i].Normal.Normalised);
            }
        }
        ...
        public static void Freeze(ModelMesh model, FreezeComponents flags)
        {
            ...
            if (freezePos) { model.Parent.Transform = model.Parent.Transform.ClearTranslation(); }
            if (freezeRot) { model.Parent.Transform = model.Parent.Transform.ClearRotation(); }
            if (freezeSca) { model.Parent.Transform = model.Parent.Transform.ClearScale(); }

            foreach (ModelMeshPart meshpart in model.MeshParts)
            {
                for (int i = 0; i < meshpart.VertexCount; i++)
                {
                    if (freezePos) { meshpart.VertexBuffer.ModifyVertexPosition(i, Vector3.TransformVector(meshpart.VertexBuffer.Data[i].Position, mPosition)); }
                    ...
                    if (freezeSca) { meshpart.VertexBuffer.ModifyVertexPosition(i, Vector3.TransformVector(meshpart.VertexBuffer.Data[i].Position, mScale)); }
                }
        ...
        {
            MungeMeshWithBone(includeChildren ? mesh.Parent.AllChildren() : new ModelBoneCollection { mesh.Parent });
        }
