...
namespace Flummery
{
    public class ViewportManager
    {
        ...
        {
            var top = new Viewport { Position = Viewport.Quadrant.TopLeft, ProjectionMode = Viewport.Mode.Orthographic, ZoomAxis = Vector3.UnitY };
            top.Camera.SetPosition(0, 15, 0);
            top.Camera.SetRotation(0, MathHelper.DegreesToRadians(-90), 0);

            var right = new Viewport { Position = Viewport.Quadrant.TopRight, ProjectionMode = Viewport.Mode.Orthographic, ZoomAxis = Vector3.UnitX };
            right.Camera.SetPosition(15, 0, 0);
            right.Camera.SetRotation(MathHelper.DegreesToRadians(90), 0, 0);

            var front = new Viewport { Position = Viewport.Quadrant.BottomLeft, ProjectionMode = Viewport.Mode.Orthographic, ZoomAxis = Vector3.UnitZ };
            front.Camera.SetPosition(0, 0, 15);

            var threedee = new Viewport { Position = Viewport.Quadrant.BottomRight };
